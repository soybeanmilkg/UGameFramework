...
#include "GameExperienceDefinition.h"
#include "GameExperienceManager.h"
#include "GameFeatureAction.h"
#include "GameFeaturesSubsystem.h"
#include "GameFeaturesSubsystemSettings.h"
#include "LogGameExperience.h"
...
namespace ConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarExperienceLoadRandomDelayMin(
		...
		TEXT("This value (in seconds) will be added as a delay of load completion of the experience (along with the random value experience.chaos.ExperienceDelayLoad.RandomSecs)"),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarExperienceLoadRandomDelayRange(
		...
		TEXT("A random amount of time between 0 and this value (in seconds) will be added as a delay of load completion of the experience (along with the fixed value experience.chaos.ExperienceDelayLoad.MinSecs)"),
		ECVF_Default);
...
UGameExperienceManagerComponent::UGameExperienceManagerComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SetIsReplicatedByDefault(true);
}

void UGameExperienceManagerComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
...
void UGameExperienceManagerComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	{
		if (UGameExperienceManager::RequestToDeactivatePlugin(PluginURL))
		{
	...
	if (LoadState == EGameExperienceLoadState::Loaded)
	{
		...
		for (const TObjectPtr<UGameExperienceActionSet>& ActionSet : CurrentExperience->ActionSets)
		{
		...
		{
			UE_LOG(LogGameExperience, Error, TEXT("Actions that have asynchronous deactivation aren't fully supported yet in Lyra experiences"));
		}
...
bool UGameExperienceManagerComponent::ShouldShowLoadingScreen(FString& OutReason) const
{
	if (LoadState != EGameExperienceLoadState::Loaded)
	{
...
void UGameExperienceManagerComponent::CallOrRegister_OnExperienceLoaded_HighPriority(FOnGameExperienceLoaded::FDelegate&& Delegate)
{
...
void UGameExperienceManagerComponent::CallOrRegister_OnExperienceLoaded(FOnGameExperienceLoaded::FDelegate&& Delegate)
{
	if (IsExperienceLoaded())
	{
		Delegate.Execute(CurrentExperience);
...
void UGameExperienceManagerComponent::CallOrRegister_OnExperienceLoaded_LowPriority(FOnGameExperienceLoaded::FDelegate&& Delegate)
{
	if (IsExperienceLoaded())
	{
		Delegate.Execute(CurrentExperience);
...
const UGameExperienceDefinition* UGameExperienceManagerComponent::GetCurrentExperienceChecked() const
{
...
bool UGameExperienceManagerComponent::IsExperienceLoaded() const
{
	return LoadState == EGameExperienceLoadState::Loaded && CurrentExperience != nullptr;
}

void UGameExperienceManagerComponent::OnRep_CurrentExperience()
{
	StartExperienceLoad();
}

void UGameExperienceManagerComponent::StartExperienceLoad()
{
	check(CurrentExperience != nullptr);
...
void UGameExperienceManagerComponent::OnExperienceLoadComplete()
{
	...
	if (NumGameFeaturePluginsLoading > 0)
	{
		...
		for (const FString& PluginURL : GameFeaturePluginURLs)
		{
			UGameExperienceManager::NotifyOfPluginActivation(PluginURL);
...
void UGameExperienceManagerComponent::OnGameFeaturePluginLoadComplete(const UE::GameFeatures::FResult& Result)
{
...
void UGameExperienceManagerComponent::OnExperienceFullLoadCompleted()
{
	check(LoadState != EGameExperienceLoadState::Loaded);
...
void UGameExperienceManagerComponent::OnActionDeactivationCompleted()
{
...
void UGameExperienceManagerComponent::OnAllActionsDeactivated()
{
